...
import { exchangeSchema, exchangeUpdateSchema, } from '../utils/validation';
...
export const updateExchange = async (exchangeId, exchangeData) => {
    try {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, error: 'User not authenticated' };
        ...
        // Check permissions
        if (currentExchange.owner_id !== user.id &&
      ...
      `)
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Failed to update exchange',
        };
    }
};
// Get user's exchanges (as owner or requester)
export const getUserExchanges = async (status, limit = 20, offset = 0) => {
    try {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, error: 'User not authenticated' };
      ...
      `, { count: 'exact' })
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Failed to get user exchanges',
        };
    }
};
// Accept exchange request (owner only)
export const acceptExchange = async (exchangeId, meetingLocation, meetingDatetime) => {
    try {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, error: 'User not authenticated' };
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Failed to accept exchange',
        };
    }
};
...
export const completeExchange = async (exchangeId, rating) => {
    try {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, error: 'User not authenticated' };
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Failed to complete exchange',
        };
    }
};
...
export const cancelExchange = async (exchangeId) => {
    try {
        ...
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Failed to cancel exchange',
        };
    }
};
