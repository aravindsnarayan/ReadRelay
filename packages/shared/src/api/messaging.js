...
export const getExchangeMessages = async (exchangeId, limit = 50, offset = 0) => {
    try {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, error: 'User not authenticated' };
...
export const getUserConversations = async () => {
    try {
      ...
      `)
        ...
        const conversations = await Promise.all(exchanges.map(async (exchange) => {
            // Get latest message
...
export const markMessagesAsRead = async (exchangeId) => {
    try {
        const user = await getCurrentUser();
        if (!user) {
            return { success: false, error: 'User not authenticated' };
...
export const sendTemplateMessage = async (exchangeId, receiverId, templateType, templateData) => {
    try {
        const templates = {
